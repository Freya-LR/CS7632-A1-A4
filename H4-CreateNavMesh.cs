...
namespace GameAICourse
{
    ...
    public class CreateNavMesh
    {
        ...
        public static string StudentAuthorName = "DONGNING LI";
            ...
            )
        {
            ...
            for (int i = 0; i < obstVertCount - 2; ++i)
            {
                ...
                for (int j = i + 1; j < obstVertCount - 1; ++j)
                {
                    ...
                    for (int k = j + 1; k < obstVertCount; ++k)
                    {
                        ...
                        bool edge1InPoly = IsLineSegmentInPolygons(V1, V2, obstacles);
                        bool edge2InPoly = IsLineSegmentInPolygons(V2, V3, obstacles);
                        bool edge3InPoly = IsLineSegmentInPolygons(V3, V1, obstacles);

                        bool HasVertexBetween(Vector2Int a, Vector2Int b)
                        {
                            for (int v = 0; v < obstacleVertices.Count; v++)
                            {
                                var vertex = obstacleVertices[v];
                                if (vertex == a || vertex == b) continue;
                                if (Between(a, b, vertex))
                                    return true;
                            }
                            return false;
                        }

                        if (!edge1InPoly && HasVertexBetween(V1, V2))
                            continue;
                        if (!edge2InPoly && HasVertexBetween(V2, V3))
                            continue;
                        if (!edge3InPoly && HasVertexBetween(V3, V1))
                            continue;
                        ...
                        if (origTriangles.Count > 0 && IntersectsConvexPolygons(triPoly, origTriangles))
                        { continue; }
                        ...
                        {
                            if (vertex == V1 || vertex == V2 || vertex == V3) continue;
                            if (IsPointInsidePolygon(triPoly.getIntegerPoints(), vertex))
                            {
                                containsVertex = true;
                                break;
                            }
                        ...
                        if ((!edge1InPoly && InteriorIntersectionLineSegmentWithPolygons(V1, V2, obstacles)) ||
                            (!edge2InPoly && InteriorIntersectionLineSegmentWithPolygons(V2, V3, obstacles)) ||
                            (!edge3InPoly && InteriorIntersectionLineSegmentWithPolygons(V3, V1, obstacles)))
                        {
                            continue;
                        }
            ...
            {
                mergedAny = false;
                var newAdjPolys = new AdjacentPolygons(adjPolys);
                var polysToRemove = new HashSet<Polygon>();
                var polysToAdd = new List<Polygon>();
                var edgesToProcess = new List<CommonPolygonEdge>(adjPolys.Keys);

                // Process all edges in the original adjacency structure
                foreach (var edge in edgesToProcess)
                {
                    if (!newAdjPolys.ContainsKey(edge)) continue;
                    
                    var commonPolys = adjPolys[edge];
                    if (commonPolys.IsBarrier) continue;

                    Polygon polyA = commonPolys.AB;
                    Polygon polyB = commonPolys.BA;

                    if (polysToRemove.Contains(polyA) || polysToRemove.Contains(polyB)) continue;

                    Polygon mergedPoly = MergePolygons(polyA, polyB, edge.A, edge.B);


                    if (!IsConvex(mergedPoly.getIntegerPoints())) continue;
                    bool intersectsOthers = false;

                    foreach (var poly in navmeshPolygons)
                    {
                        if (poly == polyA || poly == polyB) continue;
                        if (IntersectsConvexPolygons(mergedPoly, new List<Polygon> { poly }))
                        {
                    ...
                    polysToRemove.Add(polyA);
                    polysToRemove.Add(polyB);
                    polysToAdd.Add(mergedPoly);
                    newAdjPolys.Remove(edge);
                    
                    newAdjPolys.AddPolygon(mergedPoly, polyA, polyB);
                    mergedAny = true;
                   
                }

                // Apply changes to navmesh polygons
                if (mergedAny)
                {
                    navmeshPolygons.RemoveAll(p => polysToRemove.Contains(p));
                    navmeshPolygons.AddRange(polysToAdd);
            ...
            // First pass: Create nodes at portal edge midpoints and build mappings
            var edgeToNodeIndex = new Dictionary<CommonPolygonEdge, int>();
            var polyToEdges = new Dictionary<Polygon, List<CommonPolygonEdge>>();
            ...
            foreach (var edge in adjPolys.Keys)
            {
                ...
                // Calculate midpoint for path node
                Vector2 ptA = ConvertToFloat(edge.A);
                Vector2 ptB = ConvertToFloat(edge.B);
                Vector2 midpoint = (ptA + ptB) * 0.5f;

                // Add to nodes and create mapping
                edgeToNodeIndex[edge] = pathNodes.Count;
                pathNodes.Add(midpoint);

                // Track which polygons use this edge
                if (!polyToEdges.ContainsKey(commonPolys.AB))
                    polyToEdges[commonPolys.AB] = new List<CommonPolygonEdge>();
                if (!polyToEdges.ContainsKey(commonPolys.BA))
                    polyToEdges[commonPolys.BA] = new List<CommonPolygonEdge>();

                polyToEdges[commonPolys.AB].Add(edge);
                polyToEdges[commonPolys.BA].Add(edge);
            }

            // Second pass: Create edges between nodes in the same polygon
            foreach (var poly in navmeshPolygons)
            {
                if (!polyToEdges.TryGetValue(poly, out var edges))
                    continue;

                // Get all node indices for this polygon
                var nodeIndices = edges.Select(edge => edgeToNodeIndex[edge]).Distinct().ToList();

                // Initialize pathEdges to accommodate all nodes
                while (pathEdges.Count <= nodeIndices.Max())
                {
                    pathEdges.Add(new List<int>());
                }

                // Fully connect all nodes within this polygon
                for (int i = 0; i < nodeIndices.Count; i++)
                {
                    int nodeA = nodeIndices[i];

                    for (int j = i + 1; j < nodeIndices.Count; j++)
                    {
                        int nodeB = nodeIndices[j];

                        // Add bidirectional connection (avoid duplicates)
                        if (!pathEdges[nodeA].Contains(nodeB))
                            pathEdges[nodeA].Add(nodeB);
                        if (!pathEdges[nodeB].Contains(nodeA))
                            pathEdges[nodeB].Add(nodeA);
                    }
                }
